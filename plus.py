...
def makeLeftShortFormVideo():
    ...
    for frame_number in frame_numbers:
        ...
        # 결과 동영상 파일 생성
        output_video_name = f'{frame_number}_left_short.mp4'
...
def makeRightShortFormVideo():
    ...
    for frame_number in frame_numbers:
        ...
        # 결과 동영상 파일 생성
        output_video_name = f'{frame_number}_right_short.mp4'
        output_video_path = os.path.join(output_short_directory, output_video_name)
...
def makeLeftLongVideo():
    ...
    for frame_number in frame_numbers:
        ...
        # 결과 동영상 파일 생성
        output_video_name = f'{frame_number}_left_long.mp4'
...
def makeRightLongVideo():
    ...
    for frame_number in frame_numbers:
        ...
        end_frame = min(int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), frame_number + 90)  # 3초 후
        # 결과 동영상 파일 생성
        output_video_name = f'{frame_number}_right_long.mp4'
        output_video_path = output_long_directory + output_video_name
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_video_path, fourcc, 30, (1920, 1080))
        # 원본 비디오를 프레임 단위로 읽어서 저장
